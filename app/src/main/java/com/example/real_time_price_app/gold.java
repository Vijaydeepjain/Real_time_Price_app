        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            JSONArray jsonArray = response.getJSONArray("GoldSilver");
                            for (int i = 0; i < jsonArray.length(); i++) {
                                JSONObject hit = jsonArray.getJSONObject(i);
                                String Name = hit.getString("Gold");
                                String imageUrl = hit.getString("pic");
                                String price = hit.getString("Silver");
                                list.add(new model(imageUrl, price, Name));
                            }
                            adaptor = new realAdaptor(gold.this, list);
